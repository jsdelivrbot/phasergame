...
function keyBinding(title, key, events) {
	k = {
		...
		keyCode: key,
		isDown: function() {
			return this.group.enabled() ? engin.input.keyboard._keys[this.keyCode()].isDown : false;
		},
		isUp: function() {
...
page = {
		...
		init: function(cb) {
			...
			$.ajax({
				...
				.done(function(data) {
					...
					if (page.versions.selectedVersion() == undefined) {
						//for some reason selectedVersion is undefined when it did not find the version
	...
	init: function(cb) {
		...
		page.settings.add(
			"keyBindings",
			page.menu.settings.keyBindings,
			[
				{
					prop: "bindings",
						...
						for (var i = 0; i < bindings.length; i++) {
							for (var k = 0; k < bindings[i].keys.length; k++) {
								page.menu.settings.keyBindings
									.bindings()
									[i].keys()
									[k].keyCode(bindings[i].keys[k]);
								//subscribe to the change event on the keys
								page.menu.settings.keyBindings
									.bindings()
									[i].keys()
									[k].keyCode.subscribe(page.settings.change.bind(this, this));
							}
						}
				...
				bindKeys: _(function() {
					for (var k = 0; k < this.bindings().length; k++) {
						...
						for (var i = 0; i < keys.length; i++) {
							...
							// events
							if (keys[i].down) {
								engin.input.keyboard._keys[keys[i].keyCode()].onDown.add(
									_(function(event) {
										if (this.enabled()) {
											event();
										}
							...
							if (keys[i].up) {
								engin.input.keyboard._keys[keys[i].keyCode()].onUp.add(
									_(function(event) {
										if (this.enabled()) {
											event();
										}
