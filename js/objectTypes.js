...
baseObject.prototype = {
	...
	inportData: function(data){ //inports obj from server format
		data = data || {};
	...
	exportData: function(data){ //exports to server format
		return {
			id: this.id,
			type: this.type,
			x: this.x,
...
doorObject.prototype = fn.combindOver(fn.duplicate(baseObject.prototype),{
	...
	step: function(){
		if(Math.abs(players.player.x-this.sprite.x) < 48 && Math.abs(players.player.y-this.sprite.y) < 48 && keyBindings.game.interact.isDown() && !map.loadingMap){
