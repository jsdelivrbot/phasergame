objects = {
	...
	getObject: function(id,type,cb){ //get obj from array
		for (var i = 0; i < this.objects.length; i++) {
			...
			server.emit('getObjects',{
				...
				for (var i = 0; i < data.length; i++) {
				 	if(this.objectLoaded(data[i].id,data[i].type)){
				 		//load it
				 		this.getObject(data[i].id,data[i].type,function(data,obj){
				 			obj.inportData(data);
							objs.push(obj);
				 		}.bind(this,data[i]))
				 	}
	...
	loadObject: function(id,type,cb){ //loads obj from the server
	...
	createObject: function(type,data,cb){ //creates in db, and loads it
		if(!this.typeExists(type)) return;
	...
	deleteObject: function(id,type){ //removes obj from db
		server.emit('objectDelete',{
			id: id,
			type: type
		})
	},
	typeExists: function(type){ //checks if obj type exists
		return this.objectTypes[type] !== undefined;
	},
	objectLoaded: function(id,type){ //checks if obj is loaded
		for (var i = 0; i < this.objects.length; i++) {
	...
	removeAllObjects: function(){ //removes all objs
		for (var i = 0; i < this.objects.length; i++) {
			this.objects[i].remove();
		};
		this.objects = [];
	...
	//server events
	objectChange: function(data){ //event from server when a objs change, "objs" is an array of objs
