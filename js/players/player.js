	...
	switch (page.menu.settings.graphics.cameraMode.peek()) {
		...
		case "smooth":
			if (this.sprite.x - engin.width / 2 - engin.camera.x < engin.width / 2) {
				x =
					engin.camera.x +
					(this.sprite.x - engin.width / 2 - engin.camera.x) / page.menu.settings.graphics.cameraSmoothSpeed.peek();
			} else {
				x = this.sprite.x - engin.width / 2;
			}
			if (this.sprite.y - engin.height / 2 - engin.camera.y < engin.height / 2) {
				y =
					engin.camera.y +
					(this.sprite.y - engin.height / 2 - engin.camera.y) / page.menu.settings.graphics.cameraSmoothSpeed.peek();
			} else {
				y = this.sprite.y - engin.height / 2;
			}
			break;
...
for (var i in Player.prototype.userData) {
	Player.prototype.__defineGetter__(
		i,
		_.partial(function(i) {
			return this.userData[i];
		}, i)
	);

	Player.prototype.__defineSetter__(
		i,
		_.partial(function(i, val) {
			this.userData[i] = val;
			this.inportData();
			return val;
		}, i)
